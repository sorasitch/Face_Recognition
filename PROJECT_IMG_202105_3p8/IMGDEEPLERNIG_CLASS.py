...
#                scale = preprocessing.StandardScaler()
#                x0=self.X_train[:,:,:,i]
#                x1=x0.reshape((numofdata, sizex * sizey)).astype('float')
#                scale.fit(x1)
#                dump(scale,self.ModelName_train +str(i)+ ".joblib")     
#                x2=scale.transform(x1)
#                _x2=x2.reshape((numofdata, sizex , sizey)).astype('float')   
#                x3[:,:,:,i]=_x2
...
#                print(x2[x2.shape[0]-1])
        ...
        INPUT_SHAPE = (sizex,sizey,sizez)
        x_train = x3.reshape((numofdata, sizex, sizey, sizez)).astype('float')
#        print(x_train)
        print(x_train.shape)
...
#                EarlyStoppingByLossVal(monitor='val_acc', value=0.998, verbose=1),
        ...
        self.print_metricsMultiClassifierDL(y_train,predictions,0.5)
...
#        print(x)
        ...
        if sizez==1 :
            ...
            x2=scale.transform(x1)
            x3=x2.reshape((numofdata, sizex , sizey)).astype('float')
            x4 = x3.reshape((numofdata, sizex, sizey, sizez)).astype('float')
            
        else :
#            x3=x
#            for i in range(sizez) :
#                scale = load(modelname +str(i)+ ".joblib") 
#                x0=x[:,:,:,i]
#                x1=x0.reshape((numofdata, sizex * sizey)).astype('float')
#                x2=scale.transform(x1)
#                _x2=x2.reshape((numofdata, sizex , sizey)).astype('float')   
...
#        print("predictions :" + str(predictions))
    ...
    def LoadModel(self
        ...
        pass
        sizez=self.SizeZ_load
#        print(self.ModelName_load + ".joblib")
        if sizez==1 :
            self.ScaleObj=load(self.ModelName_load + ".joblib") 
        else :
            self.ScaleObj=load(self.ModelName_load + ".joblib") 
#            for i in range(sizez) :
##                print(self.ModelName_load +str(i)+ ".joblib")
#                self.ScaleObj.append(load(self.ModelName_load +str(i)+ ".joblib"))
        self.DLObj= keras.models.load_model(self.ModelName_load+".hdf51",compile=False)  
#        print(len(self.ScaleObj))
...
#        print(x)
        numofdata=x.shape[0]
        sizey=self.SizeY_load
        sizex=self.SizeX_load
        sizez=self.SizeZ_load
        if sizez==1 :
            x1=x.reshape((numofdata, sizex * sizey)).astype('float') 
...
#            x3=x
##            i=0
##            for Scale in self.ScaleObj :
#            for i in range(sizez) :
#                x0=x[:,:,:,i]
#                x1=x0.reshape((numofdata, sizex * sizey)).astype('float')
##                x2=Scale.transform(x1)
#                x2=self.ScaleObj[i].transform(x1).astype('float')
##                x2=x1
#                _x2=x2.reshape((numofdata, sizex , sizey)).astype('float')   
#                x3[:,:,:,i]=_x2
##                i=i+1
